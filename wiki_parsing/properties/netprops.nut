...
function PropertySet(table) {
	local set = {};

	function DoPropertySet(set, table, append) {
		foreach (key, value in table) {
			if (typeof value == "table") {
				if (upper.match(key.slice(0, 1)) || endswith(key, "data") || endswith(key, "predictable_id")) {
					DoPropertySet(set, value, append);
				} else {
					DoPropertySet(set, value, append + key + ".");
				}
				continue;
			}
...
NetProps.GetTable(proxy, 0, property_table);
NetProps.GetTable(vehicle, 0, property_table);


local set = PropertySet(property_table);


local proxy_datamaps = [
	"m_botName",
	"m_className",
	"m_teamName",
	"m_respawnInterval",
...
foreach (datamap in proxy_datamaps) {
	if (!NetProps.HasProp(proxy, datamap)) {
		continue;
	}
	set[datamap + ": " + GetType(NetProps.GetPropType(proxy, datamap))] <- 0;
}

proxy.Kill();


local vehicle_datamaps = [
	"m_bHasGun",
	"m_hPlayer",
	...
	"m_pServerVehicle.m_vehicleSounds.pGears",
	"m_pServerVehicle.m_vehicleSounds.crashSounds",
	...
	"m_pServerVehicle.m_pStateSound",
	"m_pServerVehicle.m_pStateSoundFade",
	...
	"m_pServerVehicle.m_PassengerInfo",
	"m_nSpeed",
	"m_nRPM",
	"m_flThrottle",
	"m_nBoostTimeLeft",
...
foreach (datamap in vehicle_datamaps) {
	if (!NetProps.HasProp(vehicle, datamap)) {
		continue;
	}

	set[datamap + ": " + GetType(NetProps.GetPropType(vehicle, datamap))] <- 0;
}
